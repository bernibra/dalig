...
void print_matrix(){
	...
	for(unsigned int i=0;i<n1.nodes.size();++i){
		for(unsigned int j=0;j<n2.nodes.size();++j){
			cout << gsl_matrix_get(distance_matrix, i, j);
			if (j<n2.nodes.size()-1)
				cout << " ";
		}			
		cout << endl;
...
// print the energy/cost and the normalized energy/cost function of an alignment
void print_nullmalized_energy(void *xp, int cost_function, long degree){
...
void print_normalized_energy(void *xp, int cost_function, long degree){
	double E = 0, Epair, Enorm=0;
	int j, k, nei1, nei2, norm=0;
	set<Node *> nbr_i;
	...
	if (cost_function==1 && nullcost==1){
		Enorm=Enorm/(double)norm;
		cout << "Common normalization = " << Enorm << endl;
	}
	...
	for(i=0;i<a->matches.size();++i){
		...
		if(j!=-1 || k!=-1){
			...
			if (j != -1 && k != -1){
				...
				nbr_i = n1.nodes[j]->neighbors[1];
				s_nbr=nbr_i.size();
				cout << ":"; cout << s_nbr; cout << ",";

				nbr_i = n2.nodes[k]->neighbors[1];
				s_nbr=nbr_i.size();
				cout << s_nbr;
